<form action="${adminBase}/signInRule2/doModify.do" class="form-submit" id="station_sign_rule_modify_form_id">
    <div class="modal-dialog">
        <div class="modal-content" style="width: 120rem; left:-250px">
            <input type="hidden" name="dayGiftConfig" value="${rule.dayGiftConfig}">
...
<#--                        <td><input type="text" name="days" class="form-control digits" value="${rule.days}" oninput="value=value.replace(/[^\d]/g,'');if(value.length > 4)value=value.slice(0,4)" autocomplete="off"/></td>-->
                        <td>
                            <select name="days" class="form-control">
                                <#list 1..days as d>
                                    <option value="${d}" <#if (d == rule.days)>selected</#if>>${d}</option>
                                </#list>
                            </select>
                        </td>
                    </tr>

                    <div class="main_red_set_info"></div>
...
<script>
    requirejs(['jquery'],function(){
        ...
        var giftConfig = '${rule.dayGiftConfig}';
        ...
        $form.find("select[name='days']").change(function() {
            ...
            var ele = '';
            // var giftConfig = $form.find("input[name='dayGiftConfig']").val();
            console.log("gift config2 ==== "+giftConfig);
            var configs = [];
            if(giftConfig && giftConfig.length > 0){
                configs = JSON.parse(giftConfig);
            }
            ...
            for (let i = 0; i < day; i++) {
                ...
                html += '<td width="30%" style="width: 400rem;"><input name="score" id=giftScore'+i+' class="form-control digits require" autocomplete="off" value="'+configs[i].score+'"/></td>';
                html += '<td width="30%" style="width: 400rem;"><input name="money" id=giftMoney'+i+' class="form-control require" autocomplete="off" value="'+configs[i].cash+'"/></td>';
                html += '<td width="30%" style="width: 400rem;"><input name="betRate" id=betRate'+i+' class="form-control" autocomplete="off" value="'+configs[i].betRate+'" placeholder="<@spring.message "admin.jack.give.color.cash"/>"/></td>';
                html += '</tr>';
                ele += html;
            }
