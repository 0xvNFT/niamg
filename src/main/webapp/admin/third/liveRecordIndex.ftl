...
<#list platforms as p>platforms['${p.value}']="${p.name()}";</#list>
	...
	Fui.addBootstrapTable({
		url : '${adminBase}/liveRecord/list.do',
		...
		columns : [ {
			...
			formatter: function (value , row, index) {
				return '<a class="open-tab text-danger" data-refresh="true" href="${adminBase}/user/detail.do?proxyName=' + value + '" title="查看会员详情" id="fui_tab_detailNew">' + value + '</a>';
			}
		}, {
			field : 'gameName',
	...
	function contentFormatter(value, row, index) {
        if(row.type==2||row.type == 20 || row.type == 3 || row.type== 170){
            return '<a href="${adminBase}/liveRecord/viewDetail.do?id='+row.id+'&type='+row.type+'" target="_blank"><@spring.message "admin.deposit.table.detail"/></a>';
        }
