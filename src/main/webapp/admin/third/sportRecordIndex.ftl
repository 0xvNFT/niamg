...
$form.submit(function(){
	if(btable && !btable.resetOption){
		btable.bootstrapTable("refreshOptions",{url:'${adminBase}/sportRecord/list.do'});
		btable.resetOption=true;
	}
});
	Fui.addBootstrapTable({
		url : '${adminBase}/sportRecord/list.do',
                ...
                if (value) {
                    switch (row.platformType) {
                        ...
                        case "FB":
                            switch (value) {
                                case "0":
                                    return '<span class="label label-info"><@spring.message "fb.orders.status0"/></span>';
                                case "1":
                                    return "<@spring.message "fb.orders.status1"/>";
                                case "2":
                                    return '<@spring.message "fb.orders.status2"/>';
                                case "3":
                                    return '<@spring.message "fb.orders.status3"/>';
                                case "4":
                                    return '<@spring.message "fb.orders.status4"/>';
                                case "5":
                                    return '<@spring.message "fb.orders.status5"/>';
                            }
                            break;
                    }
                }
	...
	function contentFormatter(value, row, index) {
        if(row.type==49){
            return '<a href="${adminBase}/sportRecord/viewDetail.do?id='+row.id+'&type='+row.type+'" target="_blank"><@spring.message "admin.deposit.table.detail"/></a>';
        }
