                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tr>
                        ...
                        <td width="30%"><span>${draw.username }(${draw.degreeNmae}) </span><span
                                    ...
                                    data-clipboard-text="${draw.username}"><@spring.message "admin.withdraw.copy"/></span>
                    ...
                    <tr>
                        ...
                        <td class="text-left copy-btn-wrap">
                            <#if bankInfo.status == 2>
                                <span id="cardNo" name="cardNo">${draw.cardNo }</span>
                            </#if>
...
<#--                            <span class="text-danger h4">${dailyData.depositHandlerArtificial}（${dailyData.depositHandlerArtificialTimes }<@spring.message "admin.unit.ci"/>）</span>-->
                    ...
                    <tr>
                        <td class="text-right media-middle"><@spring.message "admin.replaceWithdraw.type"/>：</td>
                        <td><select id="payid" name="payid" class="form-control">
                                <#list replaceList as re>
                                    <option value="${re.id}" icon="${re.icon}">${re.payType}</option>
                                </#list>
                            </select>
                        </td>
                        <td class="text-right media-middle"><@spring.message "admin.replaceWithdraw.bank"/>：</td>
                        <td><select id="onlineBank" name="onlineBank" class="form-control selectpicker"
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-danger open-tab fui-close" type="button" data-refresh="true" title="<@spring.message "admin.member.info"/>"
                        url="${adminBase}/userData/index.do?username=${draw.username }&begin=${lastDrawTime}&end=${curDateTime}">
...
<script>
    $(document).ready(function () {
        var icon = $("#payid option:selected").attr("icon");  //获取选中的项
        getBankList(icon);
    })

    $("select[name='payid']").change(function () {
        var icon = $("#payid option:selected").attr("icon");  //获取选中的项
        getBankList(icon);
    });

    function getBankList(icon) {
        var html = "<option value=''>使用代付收款银行</option>";
        $.ajax({
            url: "${adminBase}/finance/withdraw/supportBankList.do",
            data: {'icon': icon},
            success: function (data) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].indexOf("${draw.bankName }") >= 0) {
                        html += "<option value='" + data[i] + "' selected>" + data[i] + "</option>";
                    } else {
                        html += "<option value='" + data[i] + "'>" + data[i] + "</option>";
                    }
                }
                $("#onlineBank").html(html);
                $('#onlineBank').selectpicker('refresh');
                $('#onlineBank').selectpicker('render');
            }
        })
    }
    ...
    requirejs(['clipboard', 'jquery', 'bootstrap', 'Fui'], function (Clipboard) {
        ...
        function confirmCommit() {
            var icon = $("#payid option:selected").attr("icon")
            var carNo = document.getElementById("cardNo").innerText;
            $.ajax({
                url: "${adminBase}/finance/withdraw/doReplaceHandle.do",
                data: $form.serialize()+"&icon="+icon+"&cardNo="+carNo.replace("+","%2B"),
                type: 'POST',
