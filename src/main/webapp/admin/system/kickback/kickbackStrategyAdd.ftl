<form action="${adminBase}/kickbackStrategy/addSave.do" class="form-submit">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close fui-close" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><@spring.message "admin.add.daily.strategy.back"/></h4>
            </div>
            <div class="modal-body">
                <table class="table table-bordered table-striped">
                    ...
                    <tr>
                        ...
                        <td><input placeholder="<@spring.message "admin.input.right.percent"/>" name="kickback" type="number" class="form-control"/></td>
                    </tr>
                    <tr>
                        <td class="text-right media-middle"><@spring.message "admin.vaild.equals.weight"/>：</td>
                        <td><input type="number" name="minBet" class="form-control"/></td>
                        <td class="text-right media-middle"><@spring.message "admin.weight.bet.num"/>：</td>
                        <td><input placeholder="<@spring.message "admin.blank.not.limited.zero"/>" type="number" name="betRate" class="form-control"/></td>
                    </tr>
                    <tr>
                        <td class="text-right media-middle"><@spring.message "admin.max.backwater.up"/>：</td>
                        <td><input type="number" name="maxBack" placeholder="<@spring.message "admin.blank.not.limited.zero"/>" class="form-control"/></td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><#list degrees as l><label class="checkbox-inline"><input type="checkbox" name="degreeIds" value="${l.id}" checked> ${l.name}</label></#list></td>
                    </tr>

                    <tr>
                        <td class="text-right media-middle"><@spring.message "admin.vaild.vip.group"/>：</td>
                        <td colspan="3"><#list groups as le><label class="checkbox-inline"><input type="checkbox" name="groupIds" value="${le.id }" checked>${le.name}</label></#list></td>
                    </tr>
                    <tr>
