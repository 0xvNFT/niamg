<form class="fui-search table-tool" method="post" id="member_guest_manager_form_id">
    <div class="form-group fui-data-wrap">
        <div class="form-inline">
            ...
            <#if permAdminFn.hadPermission("admin:accGuest:delete")>
...
<script type="text/javascript">
    requirejs(['jquery', 'bootstrap', 'Fui'], function () {
        ...
        $form.on("click", "button.batchDelete", function (e) {
            //取消原始的事件
            e.preventDefault();
            var ids = '';
            $table.find('tbody input:checkbox:checked').each(function (i, j) {
            ...
            layer.confirm('<@spring.message "admin.sure.del.selected.item"/>？', {btn: ['<@spring.message "manager.sure.name"/>', '<@spring.message "admin.member.info.cancle"/>']}, function (index) {
                $.ajax({
                    url: '${adminBase}/accGuest/deleteBatchGuest.do',
                    data: {ids: ids},
        ...
        function operateFormatter(value, row, index) {
            var str = "";
            <#if permAdminFn.hadPermission("admin:accGuest:modify")>
            str += '<a class="open-dialog" href="${adminBase}/accGuest/pwdModify.do?id=' + row.id + '""><@spring.message "admin.modify.pwd"/></a></br>';
            </#if>
            <#if permAdminFn.hadPermission("admin:accGuest:delete")>
            str += '<a class="todo-ajax" href="${adminBase}/accGuest/deleteGuest.do?id='+row.id + "&username=" + row.username +' " title="<@spring.message "admin.deposit.bank.bankCard.delOk"/>['+row.username+']？"><@spring.message "admin.menu.del"/></a>';
            </#if>
