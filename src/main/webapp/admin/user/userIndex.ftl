    ...
    <div class="form-group fui-data-wrap">
        <div class="form-inline">
            <div class="input-group">
				<input type="text" class="form-control fui-date" name="startTime" format="YYYY-MM-DD HH:mm:ss" value="${startTime}"
                       placeholder="<@spring.message "admin.startTime"/>" autocomplete="off"> <span class="glyphicon glyphicon-th form-control-feedback" aria-hidden="true"></span>
			</div>
            ...
            <#if proxyModel!=1><button class="btn btn-primary open-dialog" type="button" url="${adminBase}/user/showAddMember.do"><@spring.message "manager.vip.add"/></button></#if>
            <#if proxyModel!=4><button class="btn btn-primary open-dialog" type="button" url="${adminBase}/user/showAddProxy.do"><@spring.message "manager.proxy.add"/></button></#if>
            </#if>
        ...
        <div class="form-inline mt5px">
            ...
            <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><@spring.message "admin.bat.oper"/><span class="caret"></span></button>
                ...
                <#if permAdminFn.hadPermission("admin:moneyOpe")>
                    <li><a class="open-dialog" href="${adminBase}/finance/moneyOpe/showBatchAdd.do"><@spring.message "admin.bat.add.money"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:moneyOpe")>
                    <li><a class="open-dialog" href="${adminBase}/finance/moneyOpe/showBatchSub.do"><@spring.message "admin.bat.decode.money"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:user:update:degree")>
                    <li><a class="open-dialog" href="${adminBase}/user/showBatchModifyDegree.do"><@spring.message "admin.bat.update.level"/></a></li>
                </#if>
                <#if proxyModel!=4 && permAdminFn.hadPermission("admin:user:update:proxy")>
                    <li><a class="open-dialog" href="${adminBase}/user/showBatchModifyProxy.do"><@spring.message "admin.bat.update.upproxy"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:user:update:group")>
                    <li><a class="open-dialog" href="${adminBase}/user/showBatchModifyGroup.do"><@spring.message "admin.bat.update.group"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:user:update:status")>
                    <li><a class="open-dialog" href="${adminBase}/user/showBatchDisableStatus.do"><@spring.message "admin.bat.dis.user"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:user:update:status")>
                    <li><a class="open-dialog" href="${adminBase}/user/showBatchEnableStatus.do"><@spring.message "admin.bat.able.user"/></a></li>
                </#if>
                <#if permAdminFn.hadPermission("admin:user:update:status")>
                    <li><a class="open-dialog" href="${adminBase}/scoreHistory/showBatchAdd.do"><@spring.message "admin.bat.score"/></a></li>
                </#if>
            </ul></div></#if>
        </div>
        <div class="form-inline mt5px">
            ...
            <#if permAdminFn.hadPermission("admin:user:resetScore")>
                <a class="btn btn-primary todo-ajax" href="${adminBase}/user/scoreToZero.do" title="<@spring.message "admin.sure.all.user.score.zero"/>？"><@spring.message "admin.all.score.zero"/></a>
            </#if>
            ...
            <#if oldProxyName!=null><a class="btn btn-primary open-tab"href="${adminBase}/user/index.do?proxyName=${oldProxyName}"><@spring.message "admin.back"/></a></#if>
        </div>
    </div>
</form>
...
<#if permAdminFn.hadPermission("admin:user:export")>
<form id="member_manager_form_export_fmId" action="${adminBase}/user/export.do" target="_blank" method="post">
...
<script type="text/javascript">
    requirejs(['jquery', 'bootstrap', 'Fui'], function () {
        ...
        function localAddress(value, row, index){
            var cleanedIp = row.registerIp ? row.registerIp.replace(/\(本机地址\)/g, '') : null;
            var translatedIp = cleanedIp ? cleanedIp + " (<@spring.message 'admin.localAddress'/>)" : "";
            var s = row.registerIp ? translatedIp : "";
            s = s + "<br>";
        ...
        function userTypeFormatter(value, row, index) {
            ...
            if (value == 130) {
            	...
            	var oldProxyName = $form.find("[name='proxyName']").val();
                s = row.level + '<@spring.message "admin.proxy.level"/>&nbsp;&nbsp<a class="open-tab" href="${adminBase}/user/index.do?proxyName='+row.username + '&oldProxyName=' + oldProxyName +'">[' + (row.underNum || 0) + ']</a>';
            }
        ...
        function degreeFormatter(value, row, index) {
            ...
            if(str==null){
                value = convertToHindiNumber(value);
                //console.log("convert to hindi =====>", value);
                str = degrees[''+value];
                //console.log("hindi grp =====>", str);
            }
        ...
        function convertToHindiNumber(value) {
            var numeralMap = {
                1: "१",
                2: "२",
                3: "३",
            ...
            // Iterate through each digit and replace it with its Hindi counterpart
            var hindiNumber = "";
            for (var i = 0; i < digits.length; i++) {
                var digit = digits[i];
                if (numeralMap.hasOwnProperty(digit)) {
                    hindiNumber += numeralMap[digit];
                } else {
                    hindiNumber += digit; // If the digit is not in the numeralMap, keep it unchanged
                }
            }
        ...
        function operateFormatter(value, row, index) {
            ...
            str += '<a class="open-dialog" href="${adminBase}/user/showModifyContact.do?id=' + row.id + '" title="<@spring.message "admin.update.contact"/>"><@spring.message "admin.update.contact"/></a></br>';
            </#if>
            <#if permAdminFn.hadPermission("admin:user:updatePwd")>
            str += '<a class="open-dialog" href="${adminBase}/user/showModifyPwd.do?id=' + row.id + '""><@spring.message "manager.password.update"/></a></br>';
            </#if>
            <#if permAdminFn.hadPermission("admin:user:updatePayPwd")>
            str += '<a class="open-dialog" href="${adminBase}/user/showModifyPayPwd.do?id=' + row.id + '""><@spring.message "admin.update.pay.pass"/></a></br>';
            </#if>
            <#--if (row.type == 130) {-->
            <#--    <#if permAdminFn.hadPermission("admin:user:update:proxy")>-->
            <#--    str += '<a class="open-dialog" href="${adminBase}/user/memberToProxy.do?id=' + row.id + '""><@spring.message "admin.update.user.proxy"/></a></br>';-->
            <#--    </#if>-->
            <#--}-->
            return str;
        }
