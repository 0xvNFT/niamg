<div class="table-tool">
    ...
    <#if permAdminFn.hadPermission("admin:article:notice:add")>
        <button class="btn btn-primary open-dialog cached" url="${adminBase}/article/add.do?type=1"><@spring.message "admin.menu.add"/></button>
    </#if>
    </div>
</div>
...
<script type="text/javascript">
    requirejs(['jquery', 'bootstrap', 'Fui'], function () {
        Fui.addBootstrapTable({
            url: '${adminBase}/article/list.do?type=5',
            columns: [{
                field: 'title',
                ...
                field: 'popupStatus',
                title: Admin.announcePopupStats,
                align: 'center',
                width: '200',
                valign: 'middle',
                formatter: function (value, row, index) {
                    return row.type == 13 ? "-" : popupStatusFormatter(value, row, index);
                }
            }, {
                field: 'sortNo',
                title: Admin.sortNo,
        ...
        function articleType(value) {
            switch (value) {
                ...
                case 7:
                    ...
                    break;
                case 9:
                    return '<@spring.message "admin.windows.not"/>';
                    break;
                case 13:
        ...
        <#if permAdminFn.hadPermission("admin:article:notice:modify")>
        ...
        function popupStatusFormatter(value, row, index) {
            <#if permAdminFn.hadPermission("admin:article:news:modify")>
            return Fui.statusFormatter({
                val: value,
                url: "${adminBase}/article/changePopupStatus.do?id=" + row.id + "&popupStatus="
            });
            <#else>
            return value == 1 ? "<@spring.message "admin.disabled"/>" : "<@spring.message "admin.enable"/>";
            </#if>
        ...
        function operateFormatter(value, row, index) {
            return [
            <#if permAdminFn.hadPermission("admin:article:notice:modify")>
                '<a class="open-dialog" href="${adminBase}/article/modify.do?type=1&id=', row.id, '" title="<@spring.message "admin.deposit.bank.bankCard.modify"/>">',
                '<i class="glyphicon glyphicon-pencil"></i>', '</a>  ',
            </#if>
            <#if permAdminFn.hadPermission("admin:article:notice:delete")>
