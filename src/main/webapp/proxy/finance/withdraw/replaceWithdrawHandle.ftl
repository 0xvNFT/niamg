...
<script>
    ...
    function getBankList(icon) {
        ...
        $.ajax({
            url: "${proxyBase}/finance/withdraw/supportBankList.do",
            data: {'icon': icon},
    ...
    requirejs(['clipboard', 'jquery', 'bootstrap', 'Fui'], function (Clipboard) {
        ...
        function confirmCommit() {
            ...
            $.ajax({
                url: "${proxyBase}/finance/withdraw/doReplaceHandle.do",
                data: $form.serialize()+"&icon="+icon+"&cardNo="+document.getElementById("cardNo").innerText,
                type: 'POST',
