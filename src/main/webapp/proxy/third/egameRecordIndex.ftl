...
<div style="padding: 10px;">
	...
	<span class="text-success"><@spring.message "admin.free.game"/></span>
</div>
...
<#list platforms as p>platforms['${p.value}']="${p.name()}";</#list>
	Fui.addBootstrapTable({
		url : '${proxyBase}/egameRecord/list.do',
		...
		columns : [ {
			...
			formatter : function (value , row, index) {
				/*return '<a class="<#if onOffNewAccdetail>open-tab<#else>open-dialog</#if> text-danger" href="${proxyBase}/account/detail<#if onOffNewAccdetail>New</#if>.do?id=' + row.parentIds + '" title="<@spring.message "admin.check.member.info"/>"  id="fui_tab_detailNew">' + row.username + '</a>'*/
    ...
    function gameNameFormatter(value, row, index) {
        if (row.type == 120) {
            switch (value) {
                ...
                case '55':
                case '66':
                    return '<span class="text-success"><@spring.message "admin.jdb.bet.gtype0"/></span>';
                case '7':
                case '27':
                case '31':
                case '67':
                    return '<span class="text-primary"><@spring.message "admin.jdb.bet.gtype7"/></span>';
                case '9':
                case '22':
                case '30':
                case '56':
                    return '<span class="text-danger"><@spring.message "admin.jdb.bet.gtype9"/></span>';
                case '12':
                    return '<span class="text-default"><@spring.message "admin.jdb.bet.gtype12"/></span>';
                case '18':
                    return '<span class="text-default"><@spring.message "admin.jdb.bet.gtype18"/></span>';
            }
        }
    }
});
