$(function() {
	requirejs(['jquery', 'bootstrap', 'Fui'], function() {
		$selctpicker.change(function() {
			console.log("selctpicker change ===>>>");
			var val = $(this).val();
			if (val && val !== '') {
				$secondGameValues.val(val.join(","));
				console.log(" is not null...... = ",document.getElementById("gameCodeArr_id").value);
			} else {
		...
		$type.change(function() {
			...
			var url = baseUrl + '/native/v2/getGame.do?type=' + type;
			console.log("game type change v = " + v);
			//获取某类游戏数据
			$.ajax({
				type: "GET",
				url: url,
		...
		$topGame.change(function() {
			...
			console.log("isListGame = " + isListGame);
						...
						if (result.success) {
							...
							$.each(result.content, function(key, value) {
								if (gameList.length > 0 && gameList.includes(value.lapisId)) {
									$secondGame.append("<option selected='selected' jumplink='" + value.finalRelatveUrl + "' gameimg='" +
									...
									$secondGame.append("<option jumplink='" + value.finalRelatveUrl + "' gameimg='" +
										value.buttonImagePath + "' value='" + value.lapisId + "'>" + value.displayName + "</option>");
							...
							$code.val($secondGame.find("option:selected").val());
							$name.val($secondGame.find("option:selected").text());
							var jumpLink = $secondGame.find("option:selected").attr("jumplink");
							var gameImg = $secondGame.find("option:selected").attr("gameimg");
							$icon.val(gameImg);
							$link.val(jumpLink);
							$isSubGame.val(1);
						}
		...
		//二级游戏change事件
		$secondGame.change(function() {
			var gameCode = $(this).val();
			var gameName = $(this).find("option:selected").text();
			var jumpLink = $(this).find("option:selected").attr("jumplink");
			var gameImg = $(this).find("option:selected").attr("gameimg");
			console.log("icon = " + gameImg + ",link = " + jumpLink);
			$name.val(gameName);
			$code.val(gameCode);
			$icon.val(gameImg);
			$link.val(jumpLink);
