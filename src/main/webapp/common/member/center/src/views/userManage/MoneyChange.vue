      ...
      <div class="cont">
        ...
        <div class="right" v-if="data.third">
          <div class="item" v-for="(val,i) in data.third" :key="i">
            <p>{{ val.name }}&nbsp;{{ $t("limitMoney") }}</p>
            <span class="num">{{ val.money }}</span>
            <div class="refresh-box">
              <p v-if="refreshing != i" class="refresh refresh-text" @click="getBalance(val.platform, i)">
                {{ $t("refresh") }}
              </p>
              <img v-else class="refresh refresh-loading" src="@/assets/images/common/loading.gif" alt="" />
            </div>
          </div>
        </div>
      </div>
