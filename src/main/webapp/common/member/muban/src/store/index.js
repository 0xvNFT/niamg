...
const getters = { //实时监听state值的变化(最新状态)
  ...
  updateThirdPageHtml: state => state.thirdPageHtml,
...
const mutations = { //自定义改变state初始值的方法，这里面的参数除了state之外还可以再传额外的参数(变量或对象);
  ...
  changeThirdPageHtml: (state, data) => state.thirdPageHtml = data || '',
...
const actions = { //自定义触发mutations里函数的方法，context与store 实例具有相同方法和属性
  ...
  getThirdPageHtml: (context, data) => context.commit('changeThirdPageHtml', data),
