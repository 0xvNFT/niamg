  ...
  <div class="order-page">
    ...
    <div class="content">
      ...
      <div class="list">
        <div class="cont" v-for="(item, index) in tableData" :key='index'>
          <div class="cont-show">
            <p>{{ item.orderId }}{{ typeFun(item.platformType) }}</p>
            <p>{{ item.gameName }}</p>
             <div class="doubleP">
                <p>
                  {{ item.username }}
              ...
              <p>{{ item.bettingTime }}</p>
              <p>{{ dataFun(item.odds) }}</p>
            <p>{{ item.winMoney | amount }}</p>
            <p>{{ item.realBettingMoney || '-' | amount }}</p>
            <p>{{ dataFun(item.bettingMoney) }}</p>
            <p>{{ item.playName }}</p>
            <template>
              <p class="lookInfo" @click.stop="lookInfo(item.bettingContent)" v-if="item.bettingContent"><span
                  @click.stop="lookInfo(item.bettingContent)"> {{ $t("lookInfo") }}</span></p>
              <p v-else>-</p>
            </template>
    ...
    <div class="statistics">
      <div class="line" v-if="data.aggsData">
        ...
        <div style="margin-left: 20px">{{ $t("realBettingMoney") }}：{{ data.aggsData.realBettingMoneyCount || 0 | amount
        }}
        </div>
...
export default {
  ...
  computed: {
    ...
    platformArr: {
      get: function () {
        let arr = [{ name: this.$t("allPlatform"), value: '' }].concat(this.onOffBtn.esport)
        return arr
      },
      set: function () {
  ...
  methods: {
    ...
    searchMethod(isSearch) {
      ...
      this.$API.esportRecord(params).then(res => {
        if (res) {
          this.data = res
