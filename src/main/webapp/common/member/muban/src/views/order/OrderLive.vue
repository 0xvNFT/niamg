      ...
      <div class="list">
        <div class="cont" v-for="(item, index) in tableData" :key="index">
          <div class="cont-show">
            ...
            <p>{{ item.bettingMoney || 0 | amount }}</p>
            ...
            <template>
              <p class="lookInfo" @click-stop="lookInfo(item.bettingContent)" v-if="item.bettingContent">
                <span @click.stop="
                  lookInfo(item.bettingContent, item.type, item.id)
                  ">
                  {{ $t("lookInfo") }}</span>
              </p>
              <p v-else>-</p>
...
export default {
  data() {
    return {
      ...
      // startTime: '2023-08-17 00:00:00',//搜索开始时间
  ...
  methods: {
    ...
    searchMethod(isSearch) {
      ...
      this.$API.liveRecord(params).then((res) => {
        if (res) {
          this.data = res;
    ...
    lookInfo(cont, type, id) {
      ...
      if (hasChildTab.includes(type)) {
        this.$API.viewDetailLive(params).then((res) => {
          if (res) {
            window.open(res);
          }
        });
