...
public class GoogleAuthenticator {
	...
	private static final String SEED = "18?qe26wuiodmsapddapwqiqe842sdawe41dasa0faeq56lkjsdhaq78qwed1adan!8@11ntqweb%^d";
	// Java实现随机数算法
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println(":" + generateSecretKey(115L));
			System.out.println(":" + generateSecretKey(115L));
			System.out.println(":" + generateSecretKey(115L));
			System.out.println(":" + generateSecretKey(115L));
	 ...
	 * @return secret key
	 */
	public static String generateSecretKey(Long stationId) {
		SecureRandom sr = null;
		try {
			sr = SecureRandom.getInstance(RANDOM_NUMBER_ALGORITHM);
			sr.setSeed(Base64.decodeBase64(SEED + stationId + "j"));
	...
	public static boolean check_code(String secret, long code, long timeMsec) {
		...
		for (int i = -window_size; i <= window_size; ++i) {
			...
			try {
				...
				// e.printStackTrace();
				throw new BaseException("谷歌验证码错误");
				// return false;
			}
			if (hash == code) {
				return true;
