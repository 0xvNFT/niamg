...
@Controller
@RequestMapping(SystemConfig.CONTROL_PATH_PROXY + "/fishingRecord")
public class ProxyFishingRecordController extends ProxyBaseController {
	...
	public void list(ThirdSearchVo v,String proxyName) {
		...
		if (StringUtils.isEmpty(proxyName)) {
			...
			proxy = thirdCenterService.getFishingPage(v);
			v.setUsername(null);
		}
		...
		user = thirdCenterService.getFishingPage(v);
