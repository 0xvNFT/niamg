...
@Controller
@RequestMapping(SystemConfig.CONTROL_PATH_PROXY + "/chessRecord")
public class ProxyChessRecordController extends ProxyBaseController {
	...
	public String index(String begin, String end, String username, Map<String, Object> map, String proxyName) {
		...
		map.put("endTime", end);
		map.put("platforms2", PlatformType.getChessPlatform());
		map.put("platforms", PlatformType.values());
		return returnPage("/third/chessRecordIndex");
	}
	...
	public void list(ThirdSearchVo v,String proxyName) {
		...
		if (StringUtils.isEmpty(proxyName)) {
			...
			proxy = thirdCenterService.getChessPage(v);
			v.setUsername(null);
		}
		...
		user = thirdCenterService.getChessPage(v);
	...
	public String viewDetail(Long id, Integer type) {
		...
		String url = thirdCenterService.getDetailUrl(id, type, 3, SystemUtil.getStation());
