...
@Controller
@RequestMapping(SystemConfig.CONTROL_PATH_PROXY + "/lotteryRecord")
public class ProxyLotteryRecordController extends ProxyBaseController {
	...
	@RequestMapping("/list")
	 public void list(ThirdSearchVo v,String proxyName) {
		...
		if (StringUtils.isEmpty(proxyName)) {
			...
			proxy = thirdCenterService.getLotteryPage(v);
			v.setUsername(null);
		}
		...
		user = thirdCenterService.getLotteryPage(v);
