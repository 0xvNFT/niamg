...
@Controller
@RequestMapping(SystemConfig.CONTROL_PATH_PROXY + "/esportRecord")
public class ProxyEsportRecordController extends ProxyBaseController {
	...
	public String index(String begin, String end, String username, Map<String, Object> map, String proxyName) {
		...
		map.put("endTime", end);
		map.put("platforms2", PlatformType.getEsportPlatforms());
		map.put("platforms", PlatformType.values());
		return returnPage("/third/esportRecordIndex");
	}
	...
	public void list(String proxyName, ThirdSearchVo v) {
		...
		if (StringUtils.isEmpty(proxyName)) {
			...
			proxy = thirdCenterService.getEsportPage(v);
			v.setUsername(null);
		}
		...
		user = thirdCenterService.getEsportPage(v);
