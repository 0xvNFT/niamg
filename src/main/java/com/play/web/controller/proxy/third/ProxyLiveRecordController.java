...
@Controller
@RequestMapping(SystemConfig.CONTROL_PATH_PROXY + "/liveRecord")
public class ProxyLiveRecordController extends ProxyBaseController {
	...
	public String index(String begin, String end, String username, Map<String, Object> map, String proxyName) {
		...
		map.put("endTime", end);
		map.put("platforms2", PlatformType.getLivePlatform());
		map.put("platforms", PlatformType.values());
		map.put("proxyName", proxyName);
		return returnPage("/third/liveRecordIndex");
	}
	...
	@RequestMapping("/list")
	 public void list(ThirdSearchVo v,String proxyName) {
		...
		if (StringUtils.isEmpty(proxyName)) {
			...
			proxy = thirdCenterService.getLivePage(v);
			v.setUsername(null);
		}
		...
		user = thirdCenterService.getLivePage(v);
