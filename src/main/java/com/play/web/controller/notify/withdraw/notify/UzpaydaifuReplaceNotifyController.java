...
public class UzpaydaifuReplaceNotifyController extends FrontBaseController implements ReplaceNotifyWrapper {

  //  private static Logger logger = LoggerFactory.getLogger(UzpaydaifuReplaceNotifyController.class);
    ...
    public void replaceNotify(HttpServletRequest request, HttpServletResponse response, String iconCss,
      ...
      //  PrintWriter pWriter = response.getWriter();
        ...
        if(map.isEmpty()){
         ...
         //   logger.error("UZ接收回调的数据：" + requstJsonData.toString());
       //     logger.error("tpay代付成功收到回调通知，订单号：" + object.getString("merTransferId"));
        }
     //   logger.error("回调Dz代付数据:" + JSONObject.toJSONString(map));
       ...
       //     logger.error("回调uzpay代付验签成功，1、成功");
            if ("1".equals(map.get("tradeResult"))) {
                if (DistributedLockUtil.tryGetDistributedLock("onlinePay:ReplaceNotify:" + mnyDrawRecord.getUserId(), 5)) {
                    mnyDrawRecordService.dealWithdrawHandleSuccessOrfail(mnyDrawRecord.getId(), MnyDrawRecord.STATUS_SUCCESS, "Uzpay代付成功");
                }
                response.getWriter().print("success");
            } else if ("2".equals(map.get("tradeResult"))){
                if (DistributedLockUtil.tryGetDistributedLock("onlinePay:ReplaceNotify:" + mnyDrawRecord.getUserId(), 5)) {
                    mnyDrawRecordService.dealWithdrawHandleSuccessOrfail(mnyDrawRecord.getId(), MnyDrawRecord.STATUS_UNDO, "Uzpay代付失败");
                }
                response.getWriter().print("success");
            }
        }
