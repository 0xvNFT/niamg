...
public class UpidaifuReplaceNotifyController extends FrontBaseController implements ReplaceNotifyWrapper {

  //  private static Logger logger = LoggerFactory.getLogger(UpidaifuReplaceNotifyController.class);
   ...
   //     logger.error("回调upi代付数据:" + JSONObject.toJSONString(map));
        String ordernumber = map.get("pt_order");
        // 用户提款记录
    //    logger.error("upi代付成功收到回调通知，订单号：" + ordernumber);
     ...
     //   logger.error("回调upi代付签名数据:" + str);
        String sign = OnlinepayUtils.MD5(str, "utf-8").toLowerCase();
    //    logger.error("回调upi代付自加密sign:" + sign);
        if (map.get("sign").equalsIgnoreCase(sign)) {
     //       logger.error("回调upi代付验签成功，4、成功");
