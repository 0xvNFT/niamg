...
public class IpayzhifuNotifyController extends FrontBaseController implements PayNotifyWrapper {


    private static Logger logger = LoggerFactory.getLogger(IpayzhifuNotifyController.class);
    ...
    public void notify(HttpServletRequest request, HttpServletResponse response,String name,
      ...
      //  logger.error("开始进入ipay支付回调");
        PrintWriter pWriter = response.getWriter();
        SortedMap<String, Object> map = getRequestJsonData(request);
     //   logger.error("ipay支付成功收到回调通知，通知内容：" + JSONObject.toJSONString(map));
        String orderNumber = map.get("out_trade_no").toString();
     //   logger.error("ipay支付成功收到回调通知，订单号：" + orderNumber);
    ...
    //        logger.error("订单号不存在");
     ...
     //       logger.error("回调IP不在白名单内订单号:" + orderNumber);
        ...
        //金额,三方以元为单位
        BigDecimal amount=new BigDecimal(map.get("money").toString());
     //   logger.info(payName+"三方回调金额："+amount);
        // 签名校验
        if (checkSign(map,key,payName)) {
            //状态判断
            if (map.get("pay_result").toString().equalsIgnoreCase("success")) {
                    synchronized (this) {
...
//                                amount, String.format("%s在线充值成功", payName));
    ...
    private boolean checkSign(SortedMap<String,Object> map,String key,String payName){
        ...
        String signStr =bufferStr.substring(0,bufferStr.length()-1)+key;
