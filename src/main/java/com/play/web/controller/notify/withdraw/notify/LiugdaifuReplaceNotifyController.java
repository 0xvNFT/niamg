...
public class LiugdaifuReplaceNotifyController extends FrontBaseController {
    @Autowired
    ...
    //private static Logger logger = LoggerFactory.getLogger(LiugdaifuReplaceNotifyController.class);

    @NotNeedLogin
    @ResponseBody
    @RequestMapping("/liugdaifu")
    public void notify(HttpServletRequest request, HttpServletResponse response) throws IOException {
        PrintWriter pWriter = response.getWriter();
        SortedMap<String, String> map = getRequestData(request);
     //   logger.error("回调6G代付数据:" + JSONObject.toJSONString(map));
        String ordernumber = map.get("outTradeId");
        // 用户提款记录
     //   logger.error("6G代付成功收到回调通知，订单号：" + ordernumber);
       ...
       //     logger.error("回调IP不在白名单内订单号:" + ordernumber);
        ...
        map.forEach((k, v) -> {
            if (!"attach".equals(k) && !"callbackUrl".equals(k) && !"sign".equals(k) && StringUtils.isNotEmpty(v)) {
                sb.append(k).append("=").append(v).append("&");
            }
        });
        String str = sb.append("key=") + replace.getMerchantKey();
        String sign = OnlinepayUtils.MD5(str,"utf-8").toUpperCase();
       // logger.error("回调6G代付回调签名:" + map.get("sign"));
      //  logger.error("回调6G代付签名数据:" + str.toString());
     //   logger.error("回调6G代付自加密sign:" + sign);
        if (map.get("sign").equalsIgnoreCase(sign)) {
        //    logger.error("回调6G代付验签成功，200” 为成功");
            ...
            } else if ("500".equals(map.get("returnCode")) || "400".equals(map.get("returnCode"))){
                synchronized (this) {
