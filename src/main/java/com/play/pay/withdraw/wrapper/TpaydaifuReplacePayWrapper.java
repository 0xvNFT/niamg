...
public class TpaydaifuReplacePayWrapper implements ReplacePayWrapper {
    private static Logger logger = LoggerFactory.getLogger(TpaydaifuReplacePayWrapper.class);
    private static String NOTIFY_URL = "@{merchant_domain}/onlinePay/ReplaceNotify/v2/";
    private static String iconCss = "tpaydaifu";
    ...
    //    String bankAddr = params[17];
        ...
        map.put("appid", mechCode); //商户号
        map.put("orderId", orderNo); //商户订单号
        String name=cardName;
        String cardNameSign=null;
        try {
             cardNameSign= URLEncoder.encode(name,"utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        map.put("name", cardNameSign); // 持卡人姓名
        map.put("money", orderAmount); //以元为单位
        map.put("recAcc",cardNo); //银行卡号
        map.put("bankMark", bankcodes.getString(bankCode) == null ? "" : bankcodes.getString(bankCode)); // 开户行编码
        StringBuilder sb = new StringBuilder();
        map.forEach((k, v) -> {
            sb.append(k).append("=").append(v);
        });
        map.put("sn", OnlinepayUtils.MD5(sb.append("secret=") + mechKey,"utf-8").toLowerCase());
        map.put("notifyUrl", notify_url);
        map.put("name", name); // 持卡人姓名
    //    logger.error("tpay代付发起请求:" + JSONObject.toJSONString(map));
     ...
     //   logger.error("tpay代付同步返回:" + content);
    ...
    public void searchBalance(HttpServletRequest request, MnyDrawRecordService mnyDrawRecordService, StationReplaceWithDraw replaceWithDraw, MnyDrawRecord mnyDrawRecord) throws Exception {
        ...
        logger.error("tpay代付余额查询请求:" + JSONObject.toJSONString(map));
        ...
        logger.error("tpay代付返回内容:" + content);
    ...
    public void search(HttpServletRequest request, MnyDrawRecordService mnyDrawRecordService, StationReplaceWithDraw replaceWithDraw, MnyDrawRecord mnyDrawRecord) {
        ...
        String str = sb.append("secret=") + merchantKey;
        map.put("sn", OnlinepayUtils.MD5(str,"utf-8").toLowerCase());
        logger.error("tpay代付查询请求加密字符串:" + str);
        logger.error("tpay代付查询请求:" + JSONObject.toJSONString(map));
        ...
        if ("3".equals(content)) {
            throw new ParamException(BaseI18nCode.operateErrorReson, " :代付已领取");
        } else if ("4".equals(content)) {
            synchronized (this) {
                mnyDrawRecordService.dealWithdrawHandleSuccessOrfail(mnyDrawRecord.getId(), MnyDrawRecord.STATUS_SUCCESS, "tpay代付完成");
            }
        } else if ("5".equals(content)) {
            synchronized (this) {
                mnyDrawRecordService.dealWithdrawHandleSuccessOrfail(mnyDrawRecord.getId(), MnyDrawRecord.STATUS_UNDO, "tpay代付拒绝");
            }
        } else if("0".equals(content)){
            throw new ParamException(BaseI18nCode.operateErrorReson, " :代付订单不存在");
        }else if("1".equals(content)){
            throw new ParamException(BaseI18nCode.operateErrorReson, " :代付申请中");
        }else if("2".equals(content)){
            throw new ParamException(BaseI18nCode.operateErrorReson, " :代付派发中");
        }else {
            throw new ParamException(BaseI18nCode.operateErrorReson, content);
        }
    }
