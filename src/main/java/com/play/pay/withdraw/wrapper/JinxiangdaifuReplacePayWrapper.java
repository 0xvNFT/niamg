...
public class JinxiangdaifuReplacePayWrapper implements ReplacePayWrapper {
    private static Logger logger = LoggerFactory.getLogger(JinxiangdaifuReplacePayWrapper.class);
    private static String NOTIFY_URL = "@{merchant_domain}/onlinePay/ReplaceNotify/jinxiangdaifu.do";
    ...
    public JSONObject wrap(String... params) {
        ...
        map.put("appid", mechCode);
        map.put("currency_code", "THB");
        map.put("trade_no", orderNo);
        map.put("money", new BigDecimal(orderAmount).setScale(2).toString());
        map.put("payee_name", cardName);
        map.put("card_no", cardNo);
        map.put("bank_code", bankcodes.getString(bankCode) == null ? "" : bankcodes.getString(bankCode));
        map.put("bank_name", bankCode);
        map.put("bank_branch", "test");
        map.put("bank_address", "test");
        map.put("province", "test");
        map.put("city", "test");
        map.put("union_code", "test");
        map.put("bank_mobile", "test");
        ...
        String str = sb.substring(0,sb.length()-1) + mechKey;
        logger.error("金象代付请求加密字符串:" + str);
        map.put("sign", OnlinepayUtils.MD5(str,"utf-8").toLowerCase());
        map.put("sign_type", "MD5");
        logger.error("金象代付发起请求:" + JSONObject.toJSONString(map));
        ...
        if (!obj.getString("error").equalsIgnoreCase("-1")) {
        ...
        RedisAPI.addCache(orderNo,data.getString("orderid"),60*60*60);
