...
<#list platforms as p>platforms['${p.value}']="${p.name()}";</#list>
	Fui.addBootstrapTable({
		url : '${adminBase}/egameRecord/list.do',
		...
		columns : [ {
			...
			formatter : function (value , row, index) {
				/*return '<a class="<#if onOffNewAccdetail>open-tab<#else>open-dialog</#if> text-danger" href="${adminBase}/account/detail<#if onOffNewAccdetail>New</#if>.do?id=' + row.parentIds + '" title="<@spring.message "admin.check.member.info"/>"  id="fui_tab_detailNew">' + row.username + '</a>'*/
    ...
    function gameNameFormatter(value, row, index) {
        if (row.type == 120) {
            switch (value) {
                ...
                case '18':
                    return '<span class="text-default"><@spring.message "admin.chess"/></span>';
            }
        }
        return value;
    }
