<div class="table-tool">
    ...
    <#if permAdminFn.hadPermission("admin:article:material:add")>
        <button class="btn btn-primary open-dialog cached" url="${adminBase}/article/add.do?type=3"><@spring.message "admin.menu.add"/></button>
    </#if>
    </div>
</div>
...
<script type="text/javascript">
    requirejs(['jquery', 'bootstrap', 'Fui'], function () {
        Fui.addBootstrapTable({
            url: '${adminBase}/article/list.do?type=3',
            columns: [{
                field: 'title',
        ...
        function articleType(value) {
            switch (value) {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    return '<@spring.message "admin.lottery.notice"/>';
                    break;
                case 14:
                    return '<@spring.message "admin.page.windows"/>';
                    break;
                case 16:
        ...
        function operateFormatter(value, row, index) {
            return [
            <#if permAdminFn.hadPermission("admin:article:material:modify")>
                '<a class="open-dialog" href="${adminBase}/article/modify.do?type=3&id=', row.id, '" title="<@spring.message "admin.deposit.bank.bankCard.modify"/>">',
                '<i class="glyphicon glyphicon-pencil"></i>', '</a>  ',
            </#if>
            <#if permAdminFn.hadPermission("admin:article:material:delete")>
