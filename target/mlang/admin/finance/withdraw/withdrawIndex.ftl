<form class="fui-search table-tool" method="post" id="memdrawrd_form_id">
    <div class="form-group fui-data-wrap">
        <div class="form-inline">
            ...
            <#if permAdminFn.hadPermission("admin:withdraw:export")>
            ...
            <div class="form-inline mt5px">
                ...
                <select class="form-control" name="secondReview">
                    <option value="" selected><@spring.message "admin.all"/></option>
                    <option value="1"><@spring.message "admin.deposit.order.normal"/></option>
                    <option value="2"><@spring.message "admin.deposit.order.fake"/></option>
                </select>
            ...
            <div class="form-inline mt5px">
                ...
                <input type="text" class="form-control" style="width:250px;" name="remark"
                       placeholder="<@spring.message "admin.withdraw.remark"/>">
...
<div class="hidden">
    <form id="memdrawrd_export_fm_id" action="${adminBase}/finance/withdraw/export.do" method="post"
          target="_blank">
        ...
        <input type="hidden" name="secondReview"/>
    </form>
</div>
<script type="text/javascript">
    requirejs(['clipboard','jquery', 'bootstrap', 'Fui'], function (Clipboard) {
        ...
        $form.find(".exportBtn").click(function () {
            ...
            $form1.find("[name='secondReview']").val($form.find("[name='secondReview']").val());
            $form1.submit();
        });
        ...
        Fui.addBootstrapTable({
            ...
            url: '${adminBase}/finance/withdraw/list.do',
            ...
            columns: [{
                    ...
                    field: 'secondReview',
                    title: '<@spring.message "admin.deposit.order.type"/>',
                    align: 'center',
                    width: '100',
                    valign: 'middle',
                    ...
                    allSummaryFormat: function (rows, aggsData) {
                        return '<@spring.message "admin.deposit.table.all.sum"/>总计:<br/><@spring.message "admin.deposit.table.people.num"/>:';
                    }
                }, {
                    field: 'drawMoney',
                    ...
                    field: 'withdrawVirtualCoinNum',
                    title: '<@spring.message "admin.virtual.coin.num"/>',
                    align: 'center',
                    valign: 'middle',
                    formatter: moneyFormatter
                }, {
                    field: 'withdrawVirtualCoinRate',
                    ...
                    field: 'trueMoney',
                    title: '<@spring.message "admin.withdraw.table.realDrawMoney"/>',
                    align: 'center',
                    valign: 'middle',
                    formatter: moneyFormatter
                },{
                    ...
                    field: 'opUsername',
                    title: '<@spring.message "admin.withdraw.table.opUsername"/>',
                    align: 'center',
                    valign: 'middle'
            	},{
                    field: 'remark',
                    title: '<@spring.message "admin.withdraw.table.remark"/>',
                    align: 'center',
                    valign: 'middle',
        ...
        function orderTypeFormatter(value, row, index) {
            if (row.secondReview == 2) {
                var doms = [['a', {
                    classes: ["open-dialog"],
                    props: {
                        href: "${adminBase}/finance/withdraw/getBrushDetail.do?orderId=" + row.orderId,
                        id: "fui_tab_brush",
                        title: '刷单详情'
                    },
                    subDom: [
                        ['span', {
                            classes: [""],
                            text: '<@spring.message "admin.deposit.order.fake"/>',
                            styles: {'color': 'red'}
                        }],
                    ]
                }]]
                return createDynamicsDomOuterHTMLList(doms);
        ...
        function orderFormatter(value, row, index) {
            ...
            <#else>
        	s= '<a class="<#if onOffNewAccdetail>open-tab<#else>open-dialog</#if>" href="${adminBase}/user/detail<#if onOffNewAccdetail>New</#if>.do?id=' + row.userId + '" title="<@spring.message "admin.deposit.table.detail"/>" id="fui_tab_detailNew" style="overflow:scroll;"><span class="text-danger">' + row.username +'</span></a>';
            </#if>-->
            s = '<a class="open-tab text-danger" data-refresh="true" href="${adminBase}/user/detail.do?id=' + row.userId + '" title="<@spring.message "admin.deposit.table.detail"/>" id="fui_tab_detailNew" style="overflow:scroll;"><span class="text-danger">' + row.username +'</span></a>';
            if (row.first) {
        ...
        function betCheckFormatter(value, row, index) {
            <#--var check = '<a class="open-tab" title="彩票投注记录" href="${adminBase}/lotOrder/index.do?username=' + row.username + '">彩票</a>&nbsp;&nbsp;';-->
            <#--if ('${onOffReal}' == 'true') {-->
            <#--    check += '<a class="open-tab" title="真人投注记录" href="${adminBase}/realOrder/index.do?username=' + row.username + '">真人</a>&nbsp;&nbsp;';-->
            <#--}-->
            <#--if ('${onOffEgame}' == 'true') {-->
            <#--    check += '<a class="open-tab" title="电子投注记录" href="${adminBase}/egameOrder/index.do?username=' + row.username + '">电子</a><br>';-->
            <#--}-->
            <#--if ('${onOffShaba}' == 'true') {-->
            <#--    check += '<a class="open-tab" title="第三方体育投注记录" href="${adminBase}/thirdSportRecord/index.do?username=' + row.username + '">第三方体育</a>&nbsp;&nbsp;';-->
            <#--}-->
            <#--if ('${onOffSport}' == 'true') {-->
            <#--    check += '<a class="open-tab" title="体育投注记录" href="${adminBase}/sportOrder/index.do?username=' + row.username + '">体育</a>&nbsp;&nbsp;';-->
            <#--}-->
            <#--if ('${onOffChess}' == 'true') {-->
            <#--    check += '<a class="open-tab" title="棋牌投注记录" href="${adminBase}/chessOrder/index.do?username=' + row.username + '">棋牌</a>';-->
            <#--}-->
            <#--return check;-->
        }
        ...
        function operateFormatter(value, row, index) {
            ...
            if (value == 1 && row.status == 1) {
                return ['<a class="todo-ajax" href="${adminBase}/finance/withdraw/lock.do?lockFlag=2&id=', row.id, '" ><@spring.message "admin.deposit.status.be.lock"/></a>'].join('');
            } else if (value == 2 && row.status == 1 && row.type != 2) {
                if (baseInfo.username == row.opUsername) {
                    if ($("#paySwitch").val()) {
                        str += "<a class='open-dialog' href='${adminBase}/finance/withdraw/handle.do?type=2&id=" + row.id + "', ' title='代付处理'>   代付处理</a>";
                    }
                    return ['<a class="todo-ajax" href="${adminBase}/finance/withdraw/lock.do?lockFlag=1&id=', row.id, '" title="<@spring.message "admin.deposit.status.be.unlock"/>"><@spring.message "admin.deposit.status.be.unlock"/></a>  ',
                        '<a class="open-dialog" href="${adminBase}/finance/withdraw/handle.do?id=', row.id, '" title="<@spring.message "admin.withdraw.table.handle"/>"><@spring.message "admin.withdraw.table.handle"/></a>', str].join('');
                }
                return "<@spring.message "admin.deposit.status.has.bean.lock"/>";
            }else if (value == 2 && row.status == 1 && row.type == 2) {
                if ($("#cancelPay").val()) {
                    cancel += "<a class='todo-ajax' href='${adminBase}/finance/withdraw/cancelPay.do?payId=" + row.id + "', ' title='取消前请与第三方确认无法出款，出款概不负责'>   取消</a>";
                }
                return ['<a class="todo-ajax" href="${adminBase}/finance/withdraw/search.do?payId=', row.id, '" title="查询代付">查询代付</a>', cancel].join('');
            }
            return "已操作";
        }
        function copyFun(){
    ...
    function descFormatter(value, row, index) {
        if (row.status === 2 || row.status === 3) {
            if (value === undefined || value === "") {
                return '<a class="open-dialog text-danger"  href="${adminBase}/finance/withdraw/remarkModify.do?id=' + row.id + '" title="' + value + '"><@spring.message "admin.deposit.remark.add"/></a>';
            }
            var enterIndex = 6;
            if (value && value.length > enterIndex) {
                return '<a class="open-dialog"  href="${adminBase}/finance/withdraw/remarkModify.do?id=' + row.id + '" title="' + value + '">' + value.substring(0, 6) + '.....</a>';
            }
            return '<a class="open-dialog"  href="${adminBase}/finance/withdraw/remarkModify.do?id=' + row.id + '" title="' + value + '">' + value + '</a>';
        }
        return value;
    }
