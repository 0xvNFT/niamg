...
var $form=$("#chessOrderIndexFormId");
	Fui.addBootstrapTable({
		url : '${proxyBase}/chessRecord/list.do',
	...
	function contentFormatter(value, row, index) {
		if(row.type==11||row.type==29||row.type==26||row.type==41||row.type==42){
			//百胜棋牌
			return '<a href="${proxyBase}/chessRecord/viewDetail.do?id='+row.id+'&type='+row.type+'" target="_blank"><@spring.message "admin.deposit.table.detail"/></a>';
		}
