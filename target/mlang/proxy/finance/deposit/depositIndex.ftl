...
<div class="hidden">
    <form id="pay_online_rd_export_fmId" action="${proxyBase}/finance/deposit/export.do" target="_blank" method="post">
...
<script type="text/javascript">
    requirejs(['clipboard', 'jquery', 'bootstrap', 'Fui', 'bootstrapSelect'], function (Clipboard) {
        ...
        Fui.addBootstrapTable({
            ...
            showAllSummary: true,
            showFooter: true,
            url: "${proxyBase}/finance/deposit/list.do",
            columns: [{
                field: 'orderId',
        ...
        function orderFormatter(value, row, index) {
            ...
            <#else>
            s = '<a class="<#if onOffNewAccdetail>open-tab<#else>open-dialog</#if>" href="${proxyBase}/user/detail<#if onOffNewAccdetail>New</#if>.do?id=' + row.userId + '" title="<@spring.message "admin.deposit.table.detail"/>" id="fui_tab_detailNew" style="overflow:scroll;"><span class="text-danger">' + row.username + '</span></a>(' + row.degreeName + ')';
            </#if>-->
            s = '<a class="open-tab text-danger" data-refresh="true" href="${proxyBase}/user/detail.do?id=' + row.userId + '" title="<@spring.message "admin.deposit.table.detail"/>" id="fui_tab_detailNew" style="overflow:scroll;"><span class="text-danger">' + row.username + '</span></a>(' + row.degreeName + ')';
            if (row.first) {
        ...
        function moneyPayFormatter(value, row, index) {
            ...
            var str = '';
            if (row.payPlatformCode == 'USDT' && row.num !== undefined && row.rate !== undefined) {
                str = row.num + " * " + row.rate + '<br/>';
            }
            if (value > 0) {
                <#if onOffCopybtn>
                return str + '<span class="text-danger">' + Fui.toDecimal(value, 2) + '</span>' + '&nbsp;&nbsp<i class="fa fa-files-o need-copy" data-clipboard-text="' + Fui.toDecimal(value, 2) + '" style="color: #337ab7;"></i>';
                <#else>
                return str + ['<span class="text-danger">', '</span>'].join(Fui.toDecimal(value, 2));
                </#if>
            }
        ...
        function descFormatter(value, row, index) {
            if (row.status != 1 && row.bgRemark === "") {
                return '<a class="open-dialog text-danger"  href="${proxyBase}/finance/deposit/remarkModify.do?id=' + row.id + '" title="' + value + '">' + '<@spring.message "admin.deposit.remark.add"/>' + '</a>';
            }
            if (value === undefined) {
                return "-";
            }
            ...
            if (value && value.length > enterIndex) {
                return '<a class="open-dialog"  href="${proxyBase}/finance/deposit/remarkModify.do?id=' + row.id + '" title="' + value + '">' + value.substring(0, 6) + '......' + '</a>';
            }
            return '<a class="open-dialog"  href="${proxyBase}/finance/deposit/remarkModify.do?id=' + row.id + '" title="' + value + '">' + value + '</a>';
        }
        ...
        function operateFormatter(value, row, index) {
            if ((value == 1 || value == '1') && (row.status == '1' || row.status == 1)) {
                ...
                return ['<a class="todo-ajax" href="${proxyBase}/finance/deposit/lock.do?lockFlag=2&id=', row.id, '" ><@spring.message "admin.deposit.status.be.lock"/></a> ',
                    '<a class="open-dialog" href="${proxyBase}/finance/deposit/handle.do?status=3&id=', row.id, '" title="<@spring.message "admin.deposit.status.be.fail"/>"><@spring.message "admin.deposit.status.be.fail"/></a>'].join('');
                <#else>
                ...
                if (baseInfo.username == row.opUsername) {
                    return ['<a class="open-dialog" href="${proxyBase}/finance/deposit/handle.do?status=2&id=', row.id, '" title="<@spring.message "admin.deposit.status.be.success"/>"><@spring.message "admin.deposit.status.be.success"/></a> ',
                        '<a class="todo-ajax" href="${proxyBase}/finance/deposit/lock.do?lockFlag=1&id=', row.id, '" title="<@spring.message "admin.deposit.status.be.unlock"/>"><@spring.message "admin.deposit.status.be.unlock"/></a>  ',
                        '<a class="open-dialog" href="${proxyBase}/finance/deposit/handle.do?status=3&id=', row.id, '" title="<@spring.message "admin.deposit.status.be.fail"/>"><@spring.message "admin.deposit.status.be.fail"/></a>'].join('');
                }
                ...
                if (baseInfo.username == row.opUsername) {
                    if (new Date().getTime() - new Date(row.modifyDatetime).getTime() < 3600000) {
                        return ['<a class="todo-ajax" href="${proxyBase}/finance/deposit/recovery.do?id=', row.id, '" title="<@spring.message "admin.deposit.status.be.recovery.tip"/>"><@spring.message "admin.deposit.status.be.recovery"/></a>'].join('');
                    }
                }
                </#if>
            }
        ...
        function getPayCombos(type) {
            ...
            $.ajax({
                url: "${proxyBase}/finance/deposit/getPayCombos.do",
                data: {'type': type},
