<form method="post" action="${proxyBase}/login.do" unReloadTable='true' class="form-submit form-horizontal" id="login_dialog_form_id">
		...
		<div class="modal-body">
			<fieldset>
				...
				<div class="input-group input-group-lg">
					...
					<span class="input-group-addon"><img id="verifyImg" src="${proxyBase}/logVerifycode.do" border="0" width="89" height="38" alt="<@spring.message "admin.login.flush.vcode"/>"></span>
				</div>
			</fieldset>
		</div>
...
$(function() {
	...
	$("#verifyImg").click(function(){
		$(this).attr("src","${proxyBase}/logVerifycode.do?time=" + (new Date()).getTime());
		$("#loginDialog_verifyCode").val("");
	}).click();
});
</script>