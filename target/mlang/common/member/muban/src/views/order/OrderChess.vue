      ...
      <div class="list">
        <div class="cont" v-for="(item, index) in tableData" :key='index'>
          <div class="cont-show">
            ...
            <template>
              <p @click.stop="lookInfo(item.bettingContent)" v-if="item.bettingContent"><span
...
export default {
  ...
  computed: {
    ...
    platformArr: {
      get: function () {
        let arr = [{ name: this.$t("allPlatform"), value: '' }].concat(this.onOffBtn.chess)
        return arr
      },
      set: function () {
  ...
  mounted() {
    ...
    var timer = setInterval(() => {
      if (_this.onOffBtn) {
        clearInterval(timer);
        // _this.platformArr = _this.platformArr.concat(_this.onOffBtn.chess)
        // 获取数据
        _this.searchMethod()
      }
    }, 300);
  ...
  methods: {
    ...
    searchMethod(isSearch) {
      ...
      this.$API.chessRecord(params).then(res => {
        if (res) {
          this.data = res
