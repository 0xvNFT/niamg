...
export default {
  data() {
    return {
      ...
      total: 0,//总条数
      pageNumber: 1,//当前页码
      // startTime: '2023-08-10 00:00:00',//搜索开始时间
  ...
  computed: {
    ...
    platformArr: {
      get: function () {
        let arr = [{ name: this.$t("allPlatform"), value: '' }].concat(this.onOffBtn.fish)
        return arr
      },
      set: function () {
  ...
  mounted() {
    ...
    var timer = setInterval(() => {
      if (_this.onOffBtn) {
        clearInterval(timer);
        // _this.platformArr = _this.platformArr.concat(_this.onOffBtn.fish)
        // 获取数据
        _this.searchMethod()
      }
    }, 300);
  ...
  methods: {
    ...
    searchMethod(isSearch) {
      ...
      var startTime = this.date ? this.date[0] : this.startTime
      var endTime = this.date ? this.date[1] : this.endTime
      //改变提交格式
      let params = {
        'startTime': startTime,
        'endTime': endTime,
      ...
      this.$API.fishRecord(params).then(res => {
        if (res) {
          this.data = res
