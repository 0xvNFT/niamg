<template>
  <div class="floatBox">
    ...
    <el-badge
      :is-dot="!signed"
      class="item"
      v-if="userInfo.login && onOffBtn.isSignIn"
    >
      <a
        target="_blank"
        class="fixed sign"
        v-banned:click="signBtn"
        @click="signBtn()"
        :key="signUpdata"
      >
    ...
    <el-dialog
      ...
      :class="{ markModal: dialogVisible }"
      style="text-align: center"
      :modal="false"
    >
      <div class="dialog-title">
        <div class="dialog-hguan">
          <img src="../../assets/images/hguan.png" alt="" />
        </div>
      ...
      <div class="dialog_Bg">
        ...
        >
          <img
            :class="index == 6 ? 'swipe--img1' : 'swipe--img'"
            src="../../assets/images/swip.png"
            alt=""
            v-if="item.canSign"
          />
          ...
          <div :class="index == 6 ? 'bonus-item1' : 'bonus-item'">
            <div class="cashBox" v-if="onOffBtn.isShowOnSign">
              <div>
                <img src="../../assets/images/cash-icon.png" class="cashImg" />
              </div>
              <span> {{ item.cash }}</span>
            </div>
      ...
      <div class="right--drawer-box">
        <aside class="right-drawer" :class="{ show: showDrawer }">
          <div class="drawer--title">
            <h1>{{ $t("signRecourd") }}</h1>
            <img
              src="@/assets/images/da_zhuan_pan/icon_close.png"
              @click="signRecord(false)"
              alt=""
            />
          </div>
    ...
    >
      <div class="finish-body">
        <div class="imgBox">
          <img class="swipe-xuan" src="../../assets/images/swips.png" alt="" />
          <img
            class="finish-gongxi"
            src="../../assets/images/finish.png"
            alt=""
          />
          <img class="finish-yhua" src="../../assets/images/yhua.png" />
        </div>
        <div class="finish-text">
          <div class="moneyBox" v-if="onOffBtn.isShowOnSign">
            <img src="../../assets/images/cash-icon.png" />
            <span>{{ money }}</span>
          </div>
      ...
      this.$API.signRuleConfig().then((res) => {
        if (res.success) {
          ...
          for (let i = 0; i < this.dataList.length; i++) {
            ...
            //根据下标 添加图片
            if (i <= 2) {
              item.img = require(`@/assets/images/qiandao.png`);
            } else if (i >= 3 && i <= 5) {
              item.img = require(`@/assets/images/qiandao1.png`);
            } else {
              item.img = require(`@/assets/images/qiandao2.png`);
              //给第7天 长条样式
    ...
    .el-dialog {
      ...
      .finish-body {
        ...
        .finish-text {
          ...
          align-items: center;
          background: no-repeat center/100% 100% url('../../assets/images/butt.png');
          margin: auto;
          width: 216px;
          .moneyBox {
            height: 50px;
