...
// El组件的时间选择器校验
export function pickerValidate (param) {
  let pickerOption = {
    disabledDate: (time) => {
      // 未来时间置灰不可选
  ...
  if (param && param == 'onlyAllowSearchInMonth') {
    pickerOption.onPick = (picker) => {
      // 不允许跨月选择日期
      let { minDate, maxDate } = picker;
      if (minDate && maxDate) {
        let startTime = String(minDate.getMonth()) + minDate.getFullYear();
        let endTime = String(maxDate.getMonth()) + maxDate.getFullYear();
        if (endTime !== startTime) {
          // 目前只显示提示，后面进行优化成直接拦截已选的跨月时间
          Message.warning(i18n.t('onlyAllowSearchInMonth'));
        }
      }
    }
  }
